...
import {
  addCollaborationInvite as addInvite,
  removeCollaborationInvite as removeInvite,
} from "@/features/share/api/firestore-service";
...
export async function createCollaborativeChecklist(
  ...
  if (!db) {
    log(
      "WRITE",
      `${COLLECTION}/${checklist.id.slice(0, 8)}...`,
      { error: "no-db" },
  ...
  try {
    const payload = stripUndefined(checklist);
    log(
      "WRITE",
      path,
    ...
    await setDoc(doc(db, COLLECTION, checklist.id), payload);
    return true;
  } catch (e) {
    log(
      "WRITE",
  ...
  try {
    ...
    log(
      ...
      path,
      {
        ...
        catCount: checklist.categories.length,
...
export async function updateCollaborativeChecklistFields(
  ...
  if (!db) {
    log(
      "UPDATE_FIELDS",
      `${COLLECTION}/${checklistId.slice(0, 8)}...`,
      { error: "no-db" },
      performance.now() - start,
    );
    return false;
    ...
    unsub = onSnapshot(
      ...
      (snap) => {
        ...
        if (snap.exists()) {
          ...
          log(
            "SNAPSHOT",
            path,
            { exists: false, status: "deleted", data: null },
            performance.now() - snapTime,
          );
      ...
      (error) => {
        log(
          "SNAPSHOT",
          path,
          { error: `${error.code}: ${error.message}`, status: "error" },
          performance.now() - start,
        );
        console.warn(
          "Collaboration doc listener error:",
          error.code,
          error.message,
        );
        if (!stopped) {
...
export async function addCollaboratorToFirestore(
  ...
  if (!db) {
    log(
      "UPDATE",
      `${COLLECTION}/${checklistId.slice(0, 8)}.../collaborators`,
      { error: "no-db" },
      performance.now() - start,
    );
    return false;
  ...
  try {
    ...
    log(
      "UPDATE",
      path,
      { addedDeviceId: deviceId.slice(0, 8), status: "ok" },
      performance.now() - start,
    );
    return true;
  } catch (e) {
...
export async function removeCollaboratorFromFirestore(
  ...
  try {
    ...
    log(
      "UPDATE",
      path,
      { removedDeviceId: deviceId.slice(0, 8), status: "ok" },
      performance.now() - start,
    );
    return true;
  } catch (e) {
