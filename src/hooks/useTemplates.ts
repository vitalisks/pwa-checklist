...
export const useTemplates = () => {
      ...
      if (existing) {
        const oldPhotoIds = new Set(
          existing.categories.flatMap((c) =>
            c.items.flatMap((i) => i.photoIds || []),
          ),
        );
        const newPhotoIds = new Set(
          template.categories.flatMap((c) =>
            c.items.flatMap((i) => i.photoIds || []),
          ),
        );
  ...
  const deleteTemplatePhoto = async (
    itemId: string,
    photoId: string,
  ): Promise<void> => {
    try {
  ...
  const startEditing = (template: Template | null) =>
